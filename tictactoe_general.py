...
while run:
    ...
    if player == -1:
        ...
        next_state = np.where(game_state == -1,2,game_state)
        state_num = state2num(next_state)
        q = np.random.uniform()
        probability_sum = 0
        print(f"State before: {state_num}")
        for i in range(P_opponent.shape[1]):
            probability_sum += P_opponent[state_num][i]
            if probability_sum >= q:
                next_state = num2state(i,n)
                break
